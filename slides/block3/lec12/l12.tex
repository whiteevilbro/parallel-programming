...
\newcommand{\thisLectureNum}{\advancedConcurrencyNum}


\title[]{Lecture \thisLectureNum: \advancedConcurrencyTopic}
...
\begin{frame}[t]{Anderson Queue Lock}
...
\begin{frame}[t,fragile]{Anderson Queue Lock}
...
\begin{frame}[t]{Memory-oriented algorithm design}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block3/12.1}}
\begin{homeworkmail}{Task 12.1}
    \begin{itemize}      
      \item Compare two \texttt{barrier} implementations
      \begin{itemize}      
          \item Counter based on TATAS lock
          \item N-element array with \texttt{RUNNING}, \texttt{READY}, \texttt{DONE} state machine
      \end{itemize}      
    \end{itemize}
\end{homeworkmail}
\end{frame}
...
\begin{frame}[t]{CLH: homework}

{\small\url{https://github.com/Svazars/parallel-programming/tree/main/hw/block3/12.2}}
\begin{homeworkmail}{Task 12.2}
    \begin{itemize}      
      \item \texttt{BadCLHLock} uses local node for spinning which leads to subtle concurrent errors. 
      \item Find and explain at least one problem in provided source code.
    \end{itemize}
\end{homeworkmail}
...
\begin{frame}[t]{Abortable queue locks: solutions}
